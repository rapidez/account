<div v-if="data.customer.orders.items.length" class="max-w-full">
    <div v-for="order in data.customer.orders.items" class="border-b border-t bg-white mb-6 sm:rounded-sm sm:border sm:mb-4">
        <div class="flex bg rounded-t-sm border-b p-4 mb-4 max-xl:flex-col xl:grid xl:grid-cols-5 xl:gap-x-6 xl:p-6 xl:items-center">
            <dl class="gap-y-3 gap-x-6 text-sm grid grid-cols-4 xl:col-span-3">
                <div class="max-xl:col-span-2">
                    ...
                    <dd class="mt-1 text-muted" data-testid="masked">
                        <a :href="window.url('/account/order/'+order.number)" class="underline hover:no-underline">
                            @{{ order.number }}
                        </a>
                    </dd>
                </div>
                ...
                <div class="max-xl:col-span-2">
                    ...
                    <dd class="mt-1 text-muted">@{{ window.price(order.total.grand_total.value) }}</dd>
                </div>
                <div class="max-xl:col-span-2">
            ...
            <div class="col-span-2 flex items-center justify-end space-x-4 max-xl:w-full max-xl:mt-3">
                <a :href="window.url('/account/order/'+order.number)" class="flex items-center justify-center rounded-md border bg-white text p-2" data-testid="order-id">
        ...
        <div class="divide-y">
            <x-rapidez::product-table v-for="item in order.items" class="px-4 py-2">
                ...
                <x-slot:price>@{{ window.price(item.product_sale_price.value) }}</x-slot:price>
                <x-slot:subtotal>@{{ window.price(item.product_sale_price.value * item.quantity_ordered) }}</x-slot:subtotal>
        ...
        <div class="flex w-full justify-end px-4 mb-4">
            <graphql-mutation
                ...
                v-slot="{ mutate }"
            >
                <form v-on:submit.prevent="mutate" class="mt-4">
