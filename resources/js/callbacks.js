...
Vue.prototype.registerCallback = async function (changes, response) {
    await InteractWithUser.methods.login(changes.email, changes.password)
    await InteractWithUser.methods.refreshUser()
}

Vue.prototype.refreshUserInfoCallback = async function (changes, response) {
    await InteractWithUser.methods.refreshUser()
}

Vue.prototype.reorderCallback = async function (changes, response) {
